...
def envoyer_email(participant):
    ...
    smtp_username = 'your_address_mail'
    smtp_password = 'appli_password'
...
@app.route('/modifier_p/<int:id_participant>', methods=['POST'])
def modifier_valeur_participant(id_participant):
    nouvelle_valeur = float(request.form.get('nouvelleValeurParticipant'))
...
@app.route('/modifier_m/<int:id_mot>', methods=['POST'])
def modifier_valeur_prix(id_mot):
    nouvelle_valeur = float(request.form.get('nouvelleValeurMot'))

    update_query = "UPDATE mots_interdit SET prix = prix + %s WHERE id = %s"
    data = (nouvelle_valeur, id_mot)

    cursor.execute(update_query, data)
    conn.commit()

    return jsonify(message="Prix modifiée avec succès.", type="success")
